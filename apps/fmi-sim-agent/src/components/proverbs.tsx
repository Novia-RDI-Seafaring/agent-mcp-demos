...
export function ProverbsCard({ state, setState }: ProverbsCardProps) {
  const proverbs = Array.isArray(state.proverbs)
  ? state.proverbs
  : state.proverbs?.proverbs ?? [];
  return (
    //<div className="bg-white/20 backdrop-blur-md p-8 rounded-2xl shadow-xl max-w-2xl w-full">
    <div className="bg-white p-8 rounded-2xl shadow-md max-w-2xl w-full border border-slate-100">
          ...
          >
            ...
            <button 
              onClick={() => 
                setState({
                ...state,
                proverbs: { proverbs: proverbs.filter((_, i) => i !== index) },
              })}
