...
from tools.functions.inputs import create_signal, merge_signals, data_model_to_ndarray, ndarray_to_data_model
from tools.functions.schema import FMUCollection, DataModel, FMUInfo, SimulationModel, StepProps, StepResponseAnalysis, CharacteristicPoints, AnalysisProps, Signal, PlotHttpURL
from tools.functions.information import _get_model_description, _get_all_model_descriptions, _get_fmu_names
from tools.functions.artifacts import plot_in_browser
...
FMU_PATH = os.getenv("DEFAULT_FMU_PATH", "models/fmus")  # point to a DIR, not a single .fmu

# default to .../agent/src/models/fmus
DEFAULT_FMU_DIR = PROJECT_ROOT / "models" / "fmus"
...
def get_fmu_names() -> List[str]:
    ...
    """
    # FMU_DIR = DEFAULT_FMU_DIR
    fmu_dir = Path(os.getenv("FMU_DIR", DEFAULT_FMU_DIR)).expanduser().resolve()
    print("FMU_DIR =", fmu_dir, "exists:", fmu_dir.exists(), "is_dir:", fmu_dir.is_dir())
    return _get_fmu_names(Path(fmu_dir))
...
def plot_simulation_tool(outputs: DataModel, inputs: Optional[DataModel] = None, port: int = 8051) -> PlotHttpURL:
    """
    Creates a Plotly/Dash visualization of simulation results in a browser.
    
    Args:
        outputs: DataModel containing the simulation output signals
        inputs: Optional DataModel containing the input signals (defaults to empty)
        port: Port number for the Dash server (default: 8051)
    
    Returns:
        PlotHttpURL: URL to the visualization
        
    Note:
        This starts a Dash server in a separate thread. The server will be accessible
        at the returned URL until the process terminates.
    """
    inputs_model = inputs or DataModel(timestamps=[], signals=[])
    return plot_in_browser(inputs_model, outputs, port=port)
